...
def ReadImage(ImageFolderPath):
    ...
    if os.path.isdir(ImageFolderPath):  # If path is of a folder containing images.
        ImageNames = os.listdir(ImageFolderPath)

        # Sort filenames alphabetically
        ImageNames_Sorted = sorted(ImageNames)

        for ImageName in ImageNames_Sorted:  # Getting all image's names present inside the folder.
            InputImage = cv2.imread(os.path.join(ImageFolderPath, ImageName))  # Reading images one by one.
    ...
    else:  # If it is not a folder (Invalid Path).
